<template>
  <div class="page">
    <el-row type="flex" justify="center" align="middle" class="container" v-loading="loading"
  ...
  methods: {
    login: function () {
      ...
      if (!isUsername(that.username)) {
        ...
        let data = {username: that.username, password: that.password, type: that.admin_login_type};
        //发送登陆请求
        that.loading = true;

        that.post("/uaa/manager/login/admin", data, function (resp) {
